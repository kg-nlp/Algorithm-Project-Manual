...
[🔨算法开发手册](https://kg-nlp.github.io/Algorithm-Project-Manual/数据分析/数据去重筛选.html)
[🔨个人知乎](https://www.zhihu.com/people/zhangyj-n)

## 背景
* 公司业务数据库中有成百上千万的数据,无论是业务人员还是算法开发都无法直接使用这些数据,如果能从中提取出几千条或几万条有代表性的数据,就可以启动业务探索或算法标注了
* 对文本进行聚类,可以压缩文本量级,提取少量数据可以进行few_shot或full_annotation训练
* LLM数据分析

## 工作

通过阅读⼤规模⽂本聚类相关的资料以及实践,总结了以下几点

* 大数据量的去重或叫聚类操作通常分两步:文本表征+聚类
* simhash这种局部敏感哈希算法,长文本不错,短文本不行, 得到文本指纹(1 1 0 1 0 1) 再通过海明距离计算相似度,应用步骤如下
    *  分词,hash值计算,特征向量加权,合并,降维
    *  将simhash签名分块存储,计算相似度;
    *  把所有语料进行存储在redis中,方便检索
    *  simhash最终得到的文本指纹可以当做是一个文本特征值,最后聚类度量的时候记得用海明距离
*  向量召回的本质是向量聚类，再辅以高效的向量查找，达到向量召回的目的
    * 这部分可以看下faiss或milvus的源码了解一下
* 文本向量化表征的方式有多种,tf-idf(词频-逆文档统计),Word2vec,预训练模型生成的文本向量等.
    * 向量化的方法,暗含了分布式假设,一定程度解决同义词问题
* 聚类的方法有很多,经常用的有kmeans,层级化聚簇,singlepass.
* 需要注意,我们选择的文本向量化表征的方式不同,那使用的距离度量也不同:
    *   像simhash选择海明距离,tf-idf看gensim或是sklearn自带的相似度计算,可能就是cos-sim;还有其他预训练生成的向量值,我们就参考faiss,milvus中的计算相似度的方法
    *   要注意向量空间中相似度的定义和平时做的向量相似度的定义要有区分:有可能一个是计算的欧式距离,另一个计算的是点积
    *   为了加快距离速度,可以考虑向量值的量化以及降维,这块可以参考向量搜索引擎的官方介绍和源码
*   从企业业务数据库中导出的数据,处理要聚类的文本,看一下有没有其他的字段,天然带有类别标签,可以利用;但这个标签的是从哪个维度进行的区分需要分析是否有价值
*   我们本身使用的是无监督聚类方法,结果好坏不容易评价,我们通过轮廓系数法,或是手肘法来判断各级分类的好坏即可
*   可以采用分级聚类的方法,提高效率
    * 比如一级可以基于文本字面检索,通过关键词聚类(粗粒度的聚类方式),二级多上面分好的桶进行细粒度聚类,可以通过向量表征计算相似度聚类
*   问题:
    *    短文本聚类簇的数量往往特别大
    *    聚类效果受到向量化好坏的影响,还有聚类算法的影响


## 核心代码功能介绍

整个工程名:large_scale_text_clustering,包含以下目录
> conf 是配置文件
> core 是核心逻辑函数
> dataset 是要去重前后的原数据
> db 是领域词典,停用词等
...
* 使用minibatch,使用sklearn计算的tfidf模型
* 使用hash特征进行聚类
...
* Faiss的简单使用：Flat
* 
> <font color=#FF000 >xb用来表示特征库，xq用来表示待查询的2048维向量</font>。如果沿用上面的例子，则xb就是提前存储了7030个样本的特征的“数据库”，它的shape就是7030x2048——这样的<font color=#FF000 >“数据库”在Faiss中称作Index object</font>

> IndexFlatL2:最简单的库:使用暴力L2搜索的数据库——也就是和特征库中的每个特征进行L2距离计算然后取出距离最近的那个
Index object的“训练”其实就是<font color=#FF000 >提前根据特征的分布进行聚类训练</font>,IndexFlatL2并不在列
...
* 让Faiss使用更少的内存：PQ
> Product Quantizer,这是一种有损压缩，所以这种Index进行检索的返回值只是近似的准确
...
只和特征库的一部分进行比对；和特征库的每一个特征进行比对，叫做穷举；只和部分特征进行比对，叫做IVF(倒排索引)；
...
> <font color=#FF000 >这个partition的概念，在Faiss中称之为Voronoi cells</font>；选择某个Voronoi cells然后进行检索的动作，称之为<font color=#FF000 >“probe”</font>；而在最近的“多少个”Voronoi cells中进行检索，这个“多少个”的数量称之为nprobe；在IVF Index object的属性中，就有nprobe这个属性
==IndexIVFPQ的参数中，d代表特征的维度2048，nlist代表Voronoi cells的数量，m代表subquantizers的数量（给PQ用），8代表使用8bits表示一个特征的1个sub-vector==。
...
1. 当需要绝对准确的结果时，使用Flat；比如IndexFlatL2 或者 IndexFlatIP；
2. 如果内存完全够用富裕的不行，使用HNSW；如果一般够用，使用Flat；如果有点吃紧，使用PCARx,...,SQ8；如果非常吃紧，使用OPQx_y,...,PQx；
3. 如果特征库小于1M个记录，使用"...,IVFx,..."==；如果在1M到10M之间，使用"...,IVF65536_HNSW32,..."==；如果在10M - 100M之间，使用"...,IVF262144_HNSW32,..."；如果在100M - 1B之间，使用"...,IVF1048576_HNSW32,..."。
...
[Milvus 百万向量搜索（SIFT1B）](https://juejin.cn/post/6844904034797617159)
...
* [sklearn中文社区](https://scikit-learn.org.cn/view/108.html)
* [菜菜sklearn](https://zhuanlan.zhihu.com/p/139945378)
* [一文入门Facebook开源向量检索框架Faiss](https://zhuanlan.zhihu.com/p/320653340)
* [使用Faiss进行海量特征的相似度匹配](https://cloud.tencent.com/developer/article/1770696)

## [BigCode 背后的大规模数据去重](https://mp.weixin.qq.com/s/vXqnLQKdm3xyiBIN-dIACg)

* 数据去重的好处:
    * 让训练更高效: 你可以用更少的训练步骤获得相同的，甚至是更好的性能
    * 防止可能的数据泄漏和基准污染: 数据重复会损害你的模型性能报告的公信力，并可能让所谓的改进沦为泡影。
    * 对数据集进行去重，能使其更易于学习、传输及协作。
* 作者处理的数据皆为代码,使用的算法为MinHash + LSH参数($$P$$,$$T$$,$$K$$,$$B$$,$$R$$)
    *   $$P$$哈希函数的个数或排列的个数
    *   $$T$$Jaccard 相似度阈值
    *   $$K$$K- 元组
    *   $$B$$条带数
...
>  Jaccard 相似性不具传递性，也就是说 $$A$$ 相似于 $$B$$ 且 $$B$$ 相似于 $$C$$，并不意味着 $$A$$ 相似于 $$C$$。所以这里可能会有不少假阳性。通过在 The Stack 数据集上的实验，我们发现，直接认可 LSH 选出来的相似对在很大程度上能提高下游模型的性能，同时还节省了处理时间和训练时间。因此目前我们正慢慢开始转向这种方法。但是，这个经验并不是放之四海而皆准的，如果你准备在自己的数据集上仿效我们的做法，我们建议你在此之前好好检查你的数据集及其特点，然后作出数据驱动的决策。

> 据质量过滤 (如过滤漏洞数据、毒性数据、偏见数据、模板生成的数据、个人身份数据等) 
...
![](.\数据去重筛选.assets\Snipaste_2023-08-17_15-36-06.jpg)
